...
def test_resolve_public_url_redirect_follows_share_google(monkeypatch):
    captured: dict[str, object] = {}

    class FakeResponse:
        url = "https://www.ithome.com.tw/news/173312"
    ...
    result = resolve_public_url_redirect(
        "https://share.google/JOiL3qgQw4I01tCAn",
        timeout_seconds=7,
    )

    assert result == "https://www.ithome.com.tw/news/173312"
    assert captured["url"] == "https://share.google/JOiL3qgQw4I01tCAn"
    assert captured["kwargs"]["allow_redirects"] is True
    assert captured["kwargs"]["stream"] is True
    assert captured["kwargs"]["timeout"] == 7
    assert captured["closed"] is True
